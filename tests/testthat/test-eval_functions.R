...
expect_that("eval_glm correctly dispatches models", {
  ...
  r1 <- eval_glm(m1)
  r2 <- eval_glm(m2, interval = "confidence")
...
test_that("rpart regression works", {
  ...
  expect_true(sum(abs(r2[31,] - c(0.5, 0, 0.5))) < 0.01,
...
test_that("Getting the model evaluation function works", {
  ...
  mod6 <- rpart(mileage ~ . - mpg, data = mtcars)
  library(randomForest)
  ...
  expect_true(all.equal(f1$eval_fun, eval_linear), "wrong eval function")
  expect_true(all.equal(f3$eval_fun, eval_linear), "wrong eval function")
  expect_true(all.equal(f4$eval_fun, eval_glm), "wrong eval function")
  expect_true(all.equal(f5$eval_fun, eval_glm), "wrong eval function")
  expect_true(all.equal(f6$eval_fun, eval_rpart), "wrong eval function")
  expect_true(all.equal(f7$eval_fun, eval_randomForest), "wrong eval function")
