...
```{r fig.out="40%", fig.keep = "hold", eval = FALSE}
...
- `formula_from_mod`
   - `response_var`, `explanatory_vars` -- if the `formula_from_mod` method works for your model, these will also work.
- `data_from_model`
   - `response_values` -- this is built from `formula_from_mod` and `data_from_model`, so there's no work to be done here.
...
It often happens that there is a sensible default method that covers lots of model architectures. You can try this out directly by running `mosaicModel:::data_from_model.default()` (or a similar name) on the model architecture you want to support.
...
```{r}
library(MASS)
my_mod <- qda(Species ~ Petal.Length + Petal.Width, data = iris)
```
...
So we'll have to implement this. This is typical, since `mod_eval_fun` is analogous to `predict` and there's no standard form for `predict` for the variety of model types supported by R.
...
Yes. Look at the help page for `predict.lda` to see what the argument names are. `newdata =` is often the name of the argument for specifying the model inputs, but sometimes it's `x` or `data` or whatever.

Since `lda`/`qda` is a classifier, the form of output we would like to produce is a table of probabilities for each class level for each input class. Let's look at the output of `predict`:
...
The arguments to the function are the same as for all the `mod_eval_fun` methods. The body of the function pulls out the `posterior` component, coerces it to a data frame and removes the row names.  It isn't always this easy. But once the function is available in your session, you can test it out.

```{r error = TRUE}
mosaicModel:::mod_eval_fun.lda(my_mod) %>% head(2)
```
